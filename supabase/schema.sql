...
import { type Language, type WeddingData, defaultWeddingData, loadWeddingData } from '@/lib/wedding-data';
...
const EN = {
  title: 'Geneva & Ken',
  heroText: 'Our Wedding',
  dateLabel: 'Date',
  venueLabel: 'Venue',
  locationLabel: 'Location',
  scheduleLabel: 'Schedule',
  itineraryLabel: 'Wedding Timeline',
  venueTitle: 'Venue & Travel',
  parkingTitle: 'Parking',
  seatingTitle: 'Seating Plan',
  searchTitle: 'Find Your Table',
  searchPlaceholder: 'Enter guest name',
  tableHeader: 'Table',
  seeAllTables: 'View all tables',
  guestsLabel: 'Guests',
  noResult: 'No matching guest found',
...
export default function WeddingWebsite() {
  ...
  const guestMatches = useMemo(() => {
    ...
    return data.guests.filter((guest) => {
      const value = `${guest.nameZh} ${guest.nameEn}`.toLowerCase();
      return value.includes(query);
    });
  }, [data.guests, searchTerm]);

  const selectedGuests = selectedTable ? data.guests.filter((guest) => guest.tableNumber === selectedTable) : [];
  ...
  const renderText = (zh: string, en: string) => (language === 'zh' ? zh : en);
      ...
      <header className="relative overflow-hidden border-b border-[#1d1a19]/10 bg-[#f6f3ef]">
        <div className="section-wrap flex items-center justify-between py-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="soft-button text-xs" onClick={() => setLanguage((v) => (v === 'zh' ? 'en' : 'zh'))}>
              {copy.languageLabel}
            </button>
          </div>
        </div>
          ...
          <div className="grid gap-8 px-6 py-8 sm:px-10 lg:grid-cols-[1.5fr_1fr] lg:px-12 lg:py-12">
            <div className="flex flex-col justify-between">
              <div>
                ...
                <h1 className="mt-4 text-4xl font-light tracking-[0.18em] sm:text-5xl lg:text-6xl">{data.coupleNameEn}</h1>
            ...
            <div className="rounded-[24px] border border-[#1d1a19]/10 bg-[#f7f1ee] p-6 sm:p-8">
              ...
              <div className="mt-2 text-sm leading-6 text-[#4d4542]">{language === 'zh' ? data.venueAddressZh : data.venueAddress}</div>
      ...
      <section id="schedule" className="section-wrap py-10">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {data.timeline.map((item) => (
            <div key={`${item.time}-${item.titleEn}`} className="panel p-6">
              <div className="text-xs uppercase tracking-[0.3em] text-[#8b7665]">{item.time}</div>
              <h4 className="mt-4 text-2xl font-light">{renderText(item.titleZh, item.titleEn)}</h4>
              <p className="mt-3 text-sm leading-6 text-[#4b4542]">{renderText(item.descriptionZh, item.descriptionEn)}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="panel p-6 sm:p-8">
          <div className="grid gap-8 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-4 text-base leading-7 text-[#4f4844]">{language === 'zh' ? data.venueAddressZh : data.venueAddress}</p>
            ...
            <div className="rounded-[24px] border border-[#1d1a19]/10 bg-[#f8f4f1] p-5">
              ...
              <p className="mt-4 text-sm leading-7 text-[#4a443d]">{language === 'zh' ? data.parkingInfoZh : data.parkingInfoEn}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
          <div className="panel p-5 sm:p-6">
            <div className="grid grid-cols-5 gap-3">
              {data.tables.map((table) => {
                const guestCount = data.guests.filter((guest) => guest.tableNumber === table.number).length;
                ...
                return (
                  ...
                  >
                    <div className="text-[10px] uppercase tracking-[0.18em] opacity-70">{language === 'zh' ? table.labelZh : table.labelEn}</div>
                    <div className="mt-3 text-xl font-medium">{table.number}</div>
                    <div className="mt-1 text-[11px] opacity-75">{guestCount} {copy.guestsLabel}</div>
                  </button>
                );
              })}
            </div>
              ...
              <>
                ...
                <div className="mt-5 space-y-2">
                  {tableSummary.slice(0, 8).map((table) => (
                    ...
                    >
                      <span>{language === 'zh' ? table.labelZh : table.labelEn}</span>
              ...
              guestMatches.length ? (
                guestMatches.slice(0, 10).map((guest) => (
                  <div key={guest.id} className="flex items-center justify-between rounded-2xl border border-[#1d1a19]/10 bg-[#f8f5f2] px-4 py-3">
                    ...
                    <div className="rounded-full bg-[#1d1a19] px-3 py-1 text-xs text-white">{copy.tableHeader} {guest.tableNumber}</div>
                  </div>
                ))
              ) : (
                <div className="rounded-2xl border border-dashed border-[#1d1a19]/20 px-4 py-8 text-center text-sm text-[#5b4f49]">{copy.noResult}</div>
              )
            ) : (
              <div className="rounded-2xl border border-dashed border-[#1d1a19]/20 px-4 py-8 text-center text-sm text-[#5b4f49]">{copy.searchPlaceholder}</div>
            )}
          </div>
        </div>
      </section>

      <footer className="section-wrap pb-12 pt-8 text-center text-sm text-[#5d514d]">{copy.footerText}</footer>
    </main>
  );
}
