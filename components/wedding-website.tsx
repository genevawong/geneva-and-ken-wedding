...
import { saveWeddingData, type WeddingData, defaultWeddingData, loadWeddingData } from '@/lib/wedding-data';
...
export default function AdminDashboard() {
  ...
  function updateGuest(id: string, field: 'nameZh' | 'nameEn' | 'tableNumber', value: string | number) {
    setData((current) => ({
      ...current,
      guests: current.guests.map((guest) => (guest.id === id ? { ...guest, [field]: value } : guest))
    }));
  }
  ...
  return (
    <main className="section-wrap py-10">
      ...
      {message && <div className="mb-6 rounded-2xl border border-[#1d1a19]/10 bg-white p-4 text-sm">{message}</div>}
        ...
        <section className="panel p-6">
          ...
          <div className="mt-5 space-y-4">
            {data.timeline.map((item, index) => (
              <div key={`${item.time}-${index}`} className="grid gap-3 rounded-2xl border border-[#1d1a19]/10 bg-[#f8f5f2] p-4 md:grid-cols-4">
                <label className="text-sm">
                  Time
