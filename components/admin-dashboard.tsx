...
function normalizeWeddingData(value: unknown): WeddingData {
  if (!value || typeof value !== 'object') return defaultWeddingData;

  const candidate = value as Partial<WeddingData>;

  return {
    ...defaultWeddingData,
    ...candidate,
    timeline: Array.isArray(candidate.timeline) && candidate.timeline.length ? candidate.timeline : defaultWeddingData.timeline,
    tables: Array.isArray(candidate.tables) && candidate.tables.length ? candidate.tables : defaultWeddingData.tables,
    guests: Array.isArray(candidate.guests) ? candidate.guests : defaultWeddingData.guests
  };
}
...
export async function loadWeddingData(): Promise<WeddingData> {
  if (typeof window !== 'undefined') {
    try {
      ...
      if (saved) {
        return normalizeWeddingData(JSON.parse(saved));
      }
    } catch {
  ...
  if (supabase) {
    try {
      ...
      if (!error && data?.data) {
        return normalizeWeddingData(data.data);
      }
    } catch {
