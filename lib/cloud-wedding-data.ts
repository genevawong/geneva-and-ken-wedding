...
export async function loadCloudWeddingData(): Promise<WeddingData> {
  ...
  const { data, error } = await supabase.from('wedding_settings').select('data').eq('id', 'main').maybeSingle();
...
export async function saveCloudWeddingData(data: WeddingData) {
  ...
  const { error } = await supabase.from('wedding_settings').upsert({ id: 'main', data }, { onConflict: 'id' });
